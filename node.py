...
def send_node_msg(name, packet):
        ...
        node_client_sock.sendall(packet.encode('utf-8')) # send list of body list strings in string form, encoded to bytestring
        return True

def get_recent_packet():
    global packet_count, last_packet
    curr_pc = packet_count
    while(curr_pc+1 != packet_count):
        if (curr_pc > packet_count):
            print("Packet got skipped?")
            return ""
        time.sleep(0.05)
...
class Node():
    ...
    def find_successor(self, _id):
        ...
        #3. if that successor node does not exist / not active, then? check next?

        if (_id <= self.successor[0] and _id > self.id): # is my successor the owner of the id, check range
            return self.successor[1]
        elif self.id == self.successor[0]: # is my id == my successor's (only node present) 
            return self.name
        elif _id == self.id: # is the id == my id,  when == as well this node successor to the id
            return self.name    
        else:
            fwd_name = self.find_preceding_node(_id)  # forward request to node B, preceding node (largest hash, smaller than k)
            ...
            send_node_msg(fwd_name, "@FINDS,"+str(_id)+","+self.name)

            packet = get_recent_packet()
            if packet != "":
                pargs = packet.split()
                if pargs[0] == "@RETURNS":
                    return pargs[1]
    ...
    def find_preceding_node(self, _id):
    ...
    def create(self):
        ...
        self.predecessor = (self.id, self.name) # itself
    ...
    def join(self, joiner_name): # bootstrap, update other's finger_tables as well        
        ...
        joinSuccess = send_node_msg(joiner_name, "@JOIN,"+self.name)
        if not joinSuccess: 
            return False
    ...
    def checkNodeActive(self, nodename):
        ...
        pings = 0
        while(pings != 3):
            if not send_node_msg(nodename, "@ACTIVE?,"+self.name):
                return False
    ...
    def isSuccessorActive(self):
        ...
        if not self.checkNodeActive(self.successor[1]): # if not alive
            ...
            #print("Get successor")
            self.setSuccessor(self.find_successor(self.name))
            self.informSuccessor()
            return False
        return True
    ...
    def informSuccessor(self): # tell them to update their immediate predecessor
        if self.successor[1] == "":
            return False
        send_node_msg(self.successor[1], "@HEY_S,"+self.name)
        return True
    ...
    def lookup(self, filename):
        print("Lookup called - answer query.")
        ...
        if f_id in self.keystore:
            print(f"Found in the node {self.id}")
        else: # check node closest to file's id (in charge of that file from finger table)
            successor = -1 # initially none
            # curr_id = 4, f_id = 3, successor found
            for i in range(len(self.finger_table)):
                if (self.finger_table[i][0] >= f_id):
                    successor = i
                    break

            if successor == -1:
                print("Not sure if this file exists.")
            else:
                print(f"Node {self.finger_table[successor][1]} with id {self.finger_table[successor][0]} may have the file.")
    ...
    def upload(self, filename):
        ...
        print(f"Uploading {filename} at node {self.id}")
    ...
    def listener(self):
        ...
        while True:
            ...
            t = threading.Thread(target=self.listening_thread, args=(client_sock, client_addr,))
            t.start()

        self.node_sock.close()
    

    def listening_thread(self, client_sock, client_addr):
        ...
        if pargs[0] == "@ACTIVE?" and self.active: #@ALIVE,192.168.0.1:5004
            ...
            sent = send_node_msg(sender_name, "@YESACTIVE")
            if not sent:
                client_sock.close()
                return False
            ...
            #sent = send_node_msg(sender_name, "@MYFT," + pickle.dumps(self.finger_table, 0).decode()) # using protocol 0 results in shorter strings
            sent = send_node_msg(sender_name, "@YOURS," + their_successor)
            if not sent:
                return False
        
        elif pargs[0] == "@FINDS": # your successor
            out = self.find_successor(int(pargs[1]))
            if out != "":
                send_node_msg(pargs[2], "@RETURNS,"+out)
            print("Query must have been forwarded further.")

        elif pargs[0] == "@HEY_S":
            new_pred_name = pargs[1]
            old_pred_name = self.predecessor[1]
            if old_pred_name != "": # if my old predecessor was not myself
            ...
            send_node_msg(new_pred_name, "@UPDATE_P,"+old_pred_name) # tell new predecessor to update their predecessor to this node's old predecessor
        ...
        '''
            ...
            obtained_finger_table = pickle.loads(packet[1].encode()) 
            sender_id = obtained_finger_table[0][0]
            print(sender_id)
        '''           
        client_sock.close()
        return True    
...
def main(argv):
    ...
    while(True):
        ...
        if (userin[0] == "create" and not new_node.getActive()):
            ...
            new_node.printInfo()
        # there can be two threads active at a time, one for listening, and one for sending data (initially none)

        #print(f"New node's ID is {new_node.get_id()}")
        #test_hash()

        #new_node.upload("file1.txt")
        #new_node.lookup("file1.txt")
        #new_node.lookup("file2.txt")
        #new_node.send_file()
        #new_node.leave()
